...
class SynologySpider(scrapy.Spider):
    ...
    allowed_domains = ['archive.synology.cn']
    start_urls = 'https://archive.synology.cn/download/Os'
    proxy = "http://127.0.0.1:8080"
    time = time.strftime("%Y-%m-%d",time.localtime())

    def parse_first(self, response):
        serieses = response.xpath('//html/body/main/div[2]/table/tbody/tr')
        for series in serieses[1:]: # Skip the first row of useless information in the table
    ...
    def parse(self,response):
        ...
        for each_product in products[1:]:
            ...
            firmware_synology["source"] = "official website"
